...
export default function SettingsPage() {
    ...
    // Usage state
    const [chatUsage, setChatUsage] = useState<UsageData | null>(null);
    const [examUsage, setExamUsage] = useState<UsageData | null>(null);
    const [flashcardUsage, setFlashcardUsage] = useState<UsageData | null>(null);
    const [resetLabel, setResetLabel] = useState("");

    const computeResetLabel = useCallback(() => {
        const now = new Date();
        ...
        if (hours > 0) {
            return `Os limites renovam em ${hours}h ${minutes}min`;
        }
        return `Os limites renovam em ${minutes}min`;
    }, []);
    ...
    useEffect(() => {
        ...
        async function fetchUsage() {
            try {
                const [chat, exam, flashcard] = await Promise.all([
                    getUserUsage("chat"),
                    getUserUsage("exam"),
                ...
                setChatUsage(chat);
                setExamUsage(exam);
                setFlashcardUsage(flashcard);
            } catch (err) {
                console.error("[Settings] Failed to fetch usage:", err);
            }
        }
        fetchUsage();

        // Update reset timer every 60 seconds
        setResetLabel(computeResetLabel());
    ...
    const isPro = chatUsage?.isPro ?? false;
        ...
        <div className="min-h-screen min-h-[100dvh] bg-background text-foreground flex flex-col overflow-x-hidden">
            <main className="flex-1 p-4 md:p-8 max-w-3xl mx-auto w-full space-y-8 md:space-y-12">
                ...
                <section className="space-y-4">
                    ...
                    {!isPro && chatUsage && examUsage && flashcardUsage && (
                        <div className="space-y-3">
                            <h4 className="font-semibold text-base text-foreground">Limites de uso diário</h4>
                            ...
                            {/* Reset timer */}
                            {resetLabel && (
                                <p className="text-sm text-[#8BA698]">{resetLabel}</p>
                            )}
                        </div>
                    )}
...
function UsageRow({
    ...
    let barColor = "bg-[#4A9E6B]";
    if (atLimit) barColor = "bg-[#C17D8A]";
    else if (warning) barColor = "bg-[#B89E6B]";
    ...
    return (
        <div className="bg-white dark:bg-card border border-[#E2EDE6] dark:border-border rounded-xl p-4 flex items-center gap-4">
            ...
            <span className={`min-w-[60px] text-right text-sm tabular-nums ${
                atLimit ? "text-[#C17D8A] font-bold" : "text-[#5A7565]"
            }`}>
                {used} / {limit}
            </span>
        </div>
