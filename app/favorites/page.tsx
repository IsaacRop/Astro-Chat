            ...
            <main className="flex-1 p-4 md:p-8 max-w-5xl mx-auto w-full space-y-6 md:space-y-8">
                            ...
                            <div className="bg-white border border-[#E2EDE6] rounded-2xl p-5 space-y-4 shadow-xl">
                                ...
                                <div className="flex flex-col md:flex-row gap-3">
                                    <div className="relative flex-1">
                                        <LinkIcon size={16} strokeWidth={1.5} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#8BA698]" />
                                        <input
                                            type="url"
                                            value={newFavorite.url}
                                            onChange={(e) => setNewFavorite({ ...newFavorite, url: e.target.value })}
                                            placeholder="URL (opcional)"
                                            className="w-full pl-10 pr-4 py-3 bg-[#F5F9F6] border border-[#E2EDE6] rounded-xl text-[#1E2E25] placeholder-[#8BA698] text-sm focus:outline-none focus:border-[#4A9E6B]/50 transition-colors"
                                        />
                                    </div>
                                    <select
                                        value={newFavorite.category}
                                        onChange={(e) => setNewFavorite({ ...newFavorite, category: e.target.value as BookmarkCategory })}
                                        className="px-4 py-3 bg-[#F5F9F6] border border-[#E2EDE6] rounded-xl text-[#5A7565] text-sm focus:outline-none focus:border-[#4A9E6B]/50 transition-colors appearance-none min-w-[150px]"
                                    >
                                ...
                                <div className="flex gap-3 pt-2">
                                    <button
                                        ...
                                        className="flex-1 px-4 py-2.5 rounded-xl bg-[#4A9E6B] text-white text-sm font-medium hover:bg-[#3B8558] transition-colors disabled:opacity-50"
                                    >
                ...
                {favorites.length === 0 && !isLoading && (
                    <div className="flex flex-col items-center justify-center py-20 md:py-32 text-center px-4">
                        <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-white border border-[#E2EDE6] flex items-center justify-center mb-6">
                            <Star size={32} className="md:w-10 md:h-10 text-[#8BA698]" strokeWidth={1.2} />
                        </div>
                        <h2 className="text-xl md:text-2xl font-serif font-medium text-[#1E2E25] mb-3">Sem Favoritos</h2>
                ...
                <div className="space-y-8">
                    {Object.entries(groupedFavorites).map(([category, categoryFavorites]) => (
                        <div key={category}>
                            <h3 className="text-sm font-medium uppercase tracking-wider text-[#8BA698] mb-4 flex items-center gap-2">
                                ...
                                <span className="text-[#8BA698] font-normal">({categoryFavorites.length})</span>
                            </h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                {categoryFavorites.map((favorite) => (
                                    <motion.div
                                        ...
                                        className="flex items-center gap-4 p-4 bg-white border border-[#E2EDE6] rounded-xl group hover:border-[#D0E0D6] transition-all relative"
                                    >
                                        <div
                                            className="w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0 bg-[#F5F9F6] border border-[#E2EDE6]"
                                        >
                                            {categoryIcons[favorite.category] || "⭐"}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-[#1E2E25] font-medium text-sm truncate font-serif leading-tight">{favorite.title}</p>
                                            {favorite.url && (
                                                <p className="text-[#8BA698] text-xs truncate font-mono mt-0.5">{favorite.url}</p>
                                            )}
                                        </div>
                                        <div className="flex items-center gap-1">
                                            {favorite.url && (
                                                <Link
                                                    href={favorite.url}
                                                    target="_blank"
                                                    className="p-2 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-[#EDF4EF] text-[#8BA698] hover:text-[#1E2E25] transition-all"
                                                >
                                            ...
                                            <button
                                                ...
                                                disabled={isPending}
                                                className="p-2 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-red-500/10 text-[#8BA698] hover:text-red-500 transition-all disabled:opacity-50"
                                            >
